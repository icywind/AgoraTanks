...
namespace Tanks.TankControllers
{
	...
	public class TankShooting : NetworkBehaviour
	{
		...
		[SyncVar(hook = "OnShellOverrideChanged")]
		...
		private int m_SpecialAmmoCount;
		...
		private void OnShellOverrideChanged(int newShell)
		{
			if (newShell == -1)
			{
				return;
		...
		[Command]
		private void CmdFire(Vector3 shotVector, Vector3 position, int randSeed)
		{
			// Tell clients this tank has fired
			RpcFire(m_PlayerNumber, shotVector, position, randSeed);
		}
		...
		private Shell FireVisualClientShell(Vector3 shotVector, Vector3 position, int randSeed)
		{
			...
			if (m_SpecialAmmoCount > 0)
			{
				...
				m_ShootingAudio.clip = SpecialProjectileLibrary.s_Instance.GetFireSoundForIndex(m_ShellIndex);
			}
			else
			{
		...
		private Rigidbody GetShellType()
		{
			...
			if ((m_ShellIndex != -1) && (m_SpecialAmmoCount > 0))
			{
				shellType = SpecialProjectileLibrary.s_Instance.GetProjectileDataForIndex(m_ShellIndex).projectilePrefab;
			}
